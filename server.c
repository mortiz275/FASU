...
void check_lost(){
	...
	while(current != NULL){
		if(difftime(time(NULL), current->last_ping) > LOST_THRESHOLD){
			printf("\033[33mWarning: %s has not been communicating!\n", current->client_id);
		}
		current = current->next;
	}
}
...
void *handle_client(void *arg){
	...
	if(buffer[0] == 'K'){
		printf("\033[0;32mReceived message from: %s status OK\n", &buffer[1]);
	}else if(buffer[0] == 'F'){
		printf("\033[1;31mReceived message from: %s status FIRE\n", &buffer[1]);
	}else if(buffer[0] == 'W'){
		printf("\033[1;34mReceived message from: %s status FLOOD\n", &buffer[1]);
	}else{
		printf("\033[33mWarning: Message from %s does not contain a valid status!\n", &buffer[1]);
	}
